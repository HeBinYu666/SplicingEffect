...
IsoImpact is a command-line tool that automates the functional evaluation of protein-coding splice isoforms. It integrates multi-isoform domain mapping and comparison, protein feature extraction, and feature-difference analysis into a unified automated workflow.
...
## 3. Building Domain Annotation Files for Other Ensembl Releases or Custom Annotations

For isoforms annotated in Ensembl releases other than release 110, or for custom annotation sets with matching GTF and protein FASTA files, users can use the `build_custom_domain.R` script in the `scripts/` directory to build a matching domain-coordinate CSV file. This file can then be supplied to IsoImpact with the corresponding GTF and protein FASTA files.

For this process, users need:

```text
1. A GTF file for the annotation version or custom annotation set to be analyzed.
2. Pfam/PfamScan domain-prediction results for the corresponding protein sequences.
```

We recommend using a GTF file that follows the format of the official Ensembl release 110 GTF files. The Pfam/PfamScan result file should retain the original output structure generated by Pfam/PfamScan, without manual modification.
...
Step 1. Prepare a GTF file for the target annotation version or custom annotation set.

Step 2. Run Pfam/PfamScan on the corresponding protein sequences and keep the original Pfam/PfamScan result file.
...
```bash
python IsoImpact.py \
  -i transcript_1 transcript_2 \
  -g your_annotation.gtf \
  -f your_proteins.fa \
  -d your_domain.csv \
  -o results/custom_isoforms
```

The generated output files have the same structure as those shown in the human and mouse examples above. For an older Ensembl release, use the GTF file, protein FASTA file, Pfam/PfamScan results, and generated domain-coordinate CSV file from that same release so that all transcript and protein identifiers remain consistent.
