...
The GTF file must contain CDS records and `protein_id` annotations for the selected isoforms. The protein IDs in the PfamScan result file must match the `protein_id` values in the GTF file. The PfamScan result file should retain the original output structure generated by PfamScan, without manual modification.
