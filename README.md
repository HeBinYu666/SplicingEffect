...
The GTF file must contain CDS records and `protein_id` annotations for the selected isoforms. The protein IDs in the Pfam/PfamScan result file must match the `protein_id` values in the GTF file. The Pfam/PfamScan result file should retain the original output structure generated by Pfam/PfamScan, without manual modification.
...
Step 2. Prepare Pfam/PfamScan domain-prediction results for the corresponding protein sequences. To make this release 109 example easy to run, this repository provides a small PfamScan-format result file at:

```text
docs/example_release109/NOTCH2_release109_pfam_results.txt
```

For a new analysis, run PfamScan or another Pfam-compatible domain annotation workflow on the protein sequences from the same Ensembl release and keep the original result format.
...
Rscript scripts/build_custom_domain.R \
  ...
  --pfam docs/example_release109/NOTCH2_release109_pfam_results.txt \
  --out NOTCH2_release109_domain.csv
```
...
python IsoImpact.py \
  ...
  -d NOTCH2_release109_domain.csv \
  -o results/release109_notch2
```

The generated output files have the same structure as those shown in the human and mouse examples above. For other Ensembl releases, use the GTF file, protein FASTA file, Pfam/PfamScan results, and generated domain-coordinate CSV file from the same release so that all transcript and protein identifiers remain consistent.
