...
4. The matching official EnsDb record available through AnnotationHub.
```

The GTF file is used by IsoImpact for transcript and CDS coordinates, and the official EnsDb record is used by the helper script to map PfamScan protein-domain intervals back to genomic coordinates. The protein IDs in the PfamScan result file must match the protein IDs used by the corresponding EnsDb release. The PfamScan result file should retain the original output structure generated by PfamScan, without manual modification.
...
After the command finishes, IsoImpact writes the release-specific example outputs to:
...
An example preview for this release 109 workflow can be added here after the workflow has been run.

The generated output files have the same structure as those shown in the human and mouse examples above. For other Ensembl releases, use the GTF file, protein FASTA file, PfamScan results, and generated domain-coordinate CSV file from the same release so that all transcript and protein identifiers remain consistent.
